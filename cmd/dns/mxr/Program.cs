...
namespace mxr
{
    ...
    class Program
    { // Preference càng nhỏ thì sẽ vào trước // theo thứ tự alt 0 -> n
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                // Create DNS query for MX record
        ...
        static byte[] MakeQuery(int id, string name)
        {
            ...
            data[position++] = (byte)0; data[position++] = (byte)15; // Type MX
        ...
        static void PopulateRecord(byte[] data, string domainName, Record record)
        {
            ...
            for (int i = 0; i < answerCount; ++i)
            {
                ...
                if (recordType == 15) // MX record
                {
                    int preference = (data[position] << 8) | data[position + 1];
                    position += 2; // Skip preference
                    string exchange = "";
                    position = ProcessDomainName(position, data, ref exchange);
                    record.MailExchange += $"Preference: {preference}, Exchange: {exchange}\n";
                }
                ...
                else if (recordType == 5) // CNAME record
                {
                    string cname = "";
                    position = ProcessDomainName(position, data, ref cname);
                    record.CanonicalName = cname;
                }
                else
                {
        ...
        static void DisplayRecord(Record record)
        {
            ...
            Console.WriteLine($"A Record (IP): {record.IpAddress}");
            Console.WriteLine($"CNAME Record: {record.CanonicalName}");
