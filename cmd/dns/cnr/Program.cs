...
namespace cnr
{
    public class Record
    {
        ...
        public string Cname { get; set; } = string.Empty; // Lưu thông tin bản ghi CNAME
    }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                // Tạo DNS query cho CNAME Record
        ...
        static byte[] MakeQuery(int id, string name)
        {
            ...
            data[position++] = 0x00; data[position++] = 0x05; // Type CNAME (Canonical Name)
        ...
        static void PopulateRecord(byte[] data, Record record)
        {
            ...
            for (int i = 0; i < answerCount; i++)
            {
                ...
                if (recordType == 5) // CNAME Record
                {
                    // Đọc giá trị CNAME
                    string cname = "";
                    ProcessDomainName(position, data, ref cname);
                    record.Cname += cname + "\n";
                    position += dataLength;
                }
                else
        ...
        static void DisplayRecord(Record record)
        {
            ...
            Console.WriteLine("CNAME Record(s):");

            // Lấy danh sách CNAME
            string[] cnames = record.Cname.Split('\n', StringSplitOptions.RemoveEmptyEntries);
